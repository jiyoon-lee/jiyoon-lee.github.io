...
sanity studio는 무엇인가요? sanity studio는 리액트로 만들어진 웹 어플리케이션 입니다.
데이터를 관리하기 위한 admin용 웹 어플리케이션입니다.
sanity는 로컬상에서 동작하고 있습니다. 데이터는 content lake라는 sanity에서 관리하는 클라우드상에 보관되어 있습니다.
데이터는 클라우드 어딘가에 서버에 있습니다. 데이터를 확인하고 관리하기 위해서 sanity studio를 사용합니다.
sanity studio는 다른 곳에 배포만 해두면 배포한 곳의 url만 있으면 studio를 접근할 수 있습니다.
sanity studio는 웹 어플리케이션이므로 우리가 구현하는대로 확장이 가능합니다.
우리가 더욱 간편하게 데이터를 조작하기 위해서 필요한 것이 있다면 언제든지 변경도 가능합니다.
데이터를 추가는 하였으나 어떤 포스트인지 확인하기가 어렵습니다. 일일이 클릭해서 확인하기 어렵기때문에
list preview를 우리 입맛대로 수정해보겠습니다.
